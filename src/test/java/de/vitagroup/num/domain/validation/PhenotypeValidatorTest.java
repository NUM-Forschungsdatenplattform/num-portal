...
public class PhenotypeValidatorTest {
    ...
    public void supportsSimpleAndOperation() {
        ...
        GroupExpression query = GroupExpression.builder().operator(Operator.AND).children(Arrays.asList(aqlExpression1, aqlExpression2)).build();

        Phenotype phenotype = Phenotype.builder().id(1L).name("Phenotype name").description("Two AQLs valid phenotype").query(query).build();
    ...
    public void supportsSimpleOrOperation() {
        ...
        GroupExpression query = GroupExpression.builder().operator(Operator.OR).children(Arrays.asList(aqlExpression1, aqlExpression2)).build();
    ...
    public void supportsSimpleNotOperation() {
        ...
        Phenotype phenotype = Phenotype.builder().id(1L).name("Phenotype name").description("One AQL valid phenotype").query(notAql).build();
    ...
    public void correctlyValidatesNotOperation() {
        ...
        GroupExpression query = GroupExpression.builder().operator(Operator.NOT).children(Arrays.asList(aqlExpression1, aqlExpression2)).build();

        Phenotype phenotype = Phenotype.builder().id(1L).name("Phenotype name").description("NOT operation can be applied to single AQL query")
                .query(query).build();
    ...
    public void correctlyValidatesOrOperation() {
        ...
        Phenotype phenotype = Phenotype.builder().id(1L).name("Phenotype name").description("OR operation cannot be applied to a single AQL query")
                .query(query).build();
    ...
    public void correctlyValidatesPhenotypeNameAndDescription() {
        ...
        Set<ConstraintViolation<Phenotype>> violations = validator.validate(noNameNoDescriptionPhenotype);
        assertThat(violations.isEmpty(), is(false));
        assertThat(violations.size(), is(2));

        assertThat(violations.stream().anyMatch(v -> v.getMessage().equals("Phenotype description is mandatory")), is(true));
        assertThat(violations.stream().anyMatch(v -> v.getMessage().equals("Phenotype name is mandatory")), is(true));
    }
    ...
    public void correctlyValidatesMissingAql() {
        ...
        Phenotype phenotype = Phenotype.builder().id(1L).name("Phenotype name").description("Missing aql query").query(query).build();
